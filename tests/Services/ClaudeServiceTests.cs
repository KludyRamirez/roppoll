...
public class ClaudeServiceTests
{
    ...
    public void Parse_ChoiceA_ReturnsIndex0()
    {
        ...
        var (index, reason) = ClaudeService.Parse(text);
    ...
    public void Parse_LinesInAnyOrder_StillWorks()
    {
        ...
        var (index, _) = ClaudeService.Parse(text);
        Assert.Equal(0, index);
    }
    ...
    public void Parse_MissingChoiceLine_Throws()
    {
        ...
        Assert.Throws<FormatException>(() => ClaudeService.Parse(text));
    }
    ...
    {
        Assert.Throws<FormatException>(() => ClaudeService.Parse(""));
    }
